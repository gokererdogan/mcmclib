...
class MHSamplerTest(unittest.TestCase):
    ...
    def test_dummy_hypothesis2_sampler(self):
        ...
        self.assertAlmostEqual(run.samples.samples[0].log_prior() + run.samples.samples[0].log_likelihood(), np.log(2.0))
        self.assertAlmostEqual(run.run_log.LogProbability[1], np.log(3.0))
        self.assertAlmostEqual(run.samples.samples[1].log_prior() + run.samples.samples[1].log_likelihood(), np.log(3.0))
